...
Firebaseのコンソールからウェブアプリを追加します。

1. Firebaseのコンソール (from https://firebase.google.com) を開きます
1. 先ほどの「Firebase projectを作成する」で作成したプロジェクト「FriendlyEats」をクリックします（またはFriendlyEatsが表示されていることを確認します）
1. 表示されたダッシュボードにある "ウェブ" (</>)をクリックします
1. アプリの登録でアプリのニックネームを入力します。特に指定はありません
1. 「このアプリのFirebase Hostingも設定します」にチェックを入れ、[アプルを登録]をクリックします
1. そのまま[次へ]をクリックします
1. Firebase CLIのインストールは後ほど実施しますので、そのまま[次へ]をクリックします
1. Firebase Hostingへのデプロイもスキップします。[コンソールへ進む]をクリックします

### Firebaseから設定を取得し、config.js を書き換える

1. Firebaseのコンソールの左のナビゲーションメニュー「Project Overview」横の「Settings」アイコンをクリックします
1. 「プロジェクトの設定」をクリックします
1. Settingsの 全般タブ ＞ Firebase SDK snippet ＞ 構成 を選択します
1. `const firebase` で始まる部分をコピーし、cloneしてきたソースコードのsrc/config.jsを書き換えます

### スターターアプリをインポートする

IDE（WebStorm、Atom、Sublime、Visual Studio Code ...）を使用している場合、📁FriendlyEats-Reactディレクトリを開くかインポートします。このディレクトリには、これから実装するレストラン情報と、オススメ情報を表示するアプリのチュートリアルのモックコードが含まれています。このチュートリアルを動かすために、そのディレクトリのコードを実装していく必要があります。
...
Firebase Hostingのアプリの設定を、アプリのローカルディレクトリとファイルから取得できるようにウェブアプリテンプレートを設定しました。これを実際に行うには、アプリをFirebaseプロジェクトに関連付ける必要があります。
...
その名前が示すように、whereメソッドは、条件に一致するフィールドを持つコレクション内のドキュメントを取得します。この場合、カテゴリが「点心」のレストランのみを取得しています。
...
ここまでは、すべての書き込みはアトミックで比較的単純です。エラーが発生した場合、ユーザーに再試行を促すか、アプリ自身が書き込みを自動的に再試行する可能性があります。
